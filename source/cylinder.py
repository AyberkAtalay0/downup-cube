...
def cylinder_traces(r, xs, ys, zs, h, n_slices=40, show_mesh=True, n_sub=4, surface_kw={}, line_kw={}):
    ...
    surface = go.Mesh3d(x=vertices[:, 0], y=vertices[:, 1], z=vertices[:, 2], i=triangles[:, 0], j=triangles[:, 1], k=triangles[:, 2], facecolor=(["mediumturquoise"]*(len(triangles)))[:len(triangles)], **surface_kw)
    traces = [surface]
...
fig = go.Figure()
fig.add_traces(
    cylinder_traces(3, 5, 3, 1, 8, 20, n_sub=4, line_kw=dict(line_color="black", line_width=2, hoverinfo="skip"), surface_kw=dict(showscale=False, lighting_ambient=0.99, lighting=dict(specular=0.5), hoverinfo="skip"))
)
